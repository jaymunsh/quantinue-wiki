...
> **무엇을 다루나:** 이 문서는 *운영 절차*(→ `사용가이드.md`)가 아니라, **이 위키 시스템(DocStill)이 내부에서 실제로 어떻게 동작하는지** — 어떤 부품이 무엇을 하고, 문서 한 장이 웹 위키가 되기까지 어떤 처리를 거치는지 — 를 설명한다.
> **한 줄 정의:** DocStill = *"팀원들이 각자 AI로 만든 HTML/MD 문서를, AI가 '주장(claim)' 단위로 증류·대조해, 확정된 것만 SSOT(`facts/`)에 쌓고, MkDocs로 웹 위키로 배포하는 파이프라인."*
...
flowchart LR
  ...
  subgraph wiki["wiki/ (심볼릭 링크)"]
    L1["index.md → ../documents/STATUS.md"]
...
│
├─ archive/              🗄️ 사이트에 안 나오는 파일
│  ├─ briefings/             회차별 STATUS 사본 · API 원문 백업
│  └─ 이어서시작_세션요약.md   세션 핸드오프 메모
│
...
| `documents/STATUS.md` | 현황판 (증류마다 전체 재작성) | 증류 엔진 |
