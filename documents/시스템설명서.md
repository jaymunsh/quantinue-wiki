...
> **참고:** 처리 회차·파일 해시는 `.state.json`이 기억한다(현재 값은 파일 직접 참조). 대화형(`/증류`)으로 처리한 회차도 끝나고 `mark_processed`로 같은 파일에 반영하므로, 엔진 실행이든 대화형이든 상태가 한곳에서 관리된다.
...
flowchart TB
  ...
  AG --> AG2["CLI가 <b>스스로</b> 파일 읽고<br/>STATUS/질문.md 씀<br/>(운영자는 호출만)"]
...
flowchart LR
  ...
  Q -.link.-> L2
  F -.link.-> L3
  wiki -->|"main push"| CI["GitHub Actions<br/>lint → strict build → gh-deploy"]
  CI --> GH["GitHub Pages<br/>jaymunsh.github.io/quantinue-wiki"]
```
...
- **배포 = main push** — `.github/workflows/deploy.yml`이 lint → strict build → `gh-deploy`를 자동 수행(1~2분). 검증 실패 시 배포 자체가 안 되므로 깨진 사이트가 라이브에 올라갈 수 없다. 수동 폴백: `mkdocs gh-deploy --force`.

### 7-B. 자동화 2종 + 명령 하네스

사람이 기억할 일을 시스템이 대신 지키는 장치들:

| 장치 | 무엇을 자동화 | 어디에 |
|---|---|---|
| **CI 배포** | main push → lint·strict build 통과 시에만 배포. Queued 멈춤은 Actions 수동 재실행(workflow_dispatch)으로 해결 | `.github/workflows/deploy.yml` |
| **정적 점검기** | nav 커버리지·깨진 링크·심볼릭·NFC 충돌·결정번호 연속성·안건코드 정합·운영용어·낡은 참조를 몇 초에 기계 검사. **엔진/설정(CONFIG) 분리** — 다른 위키에 가져가면 CONFIG만 수정 | `tools/lint.py` |

클로드 코드 명령 (`.claude/commands/`):

| 명령 | 하는 일 |
|---|---|
| `/증류` | 새 raw 파일 → 주장 추출·4분류 대조 → STATUS·질문 갱신 |
| `/병입 <내용>` | 확정 사실을 facts에 기록 + 결정 번호 부여 |
| `/점검` | lint.py(기계) + LLM(판단) 2단 감사 |
| `/배포` | lint·strict build → 커밋 확인 → push → Actions 배포 확인 |
| `/에이전트갱신 <이름>` | 1차 quantinue repo 브랜치를 읽기전용 참조해 **1차 역할 페이지**(`documents/에이전트/`) 갱신. 최종 구현 문서(`documents/핵심기능/`)는 대상이 아니다 |
...
│  ├─ 핵심기능/              최종 구현 구성요소 설명 (Strategist·Critic·감시·재판단·회고·집행)
│  ├─ 에이전트/              1차 역할 설명 — 동결 기록 (아카이브 탭)
│  └─ facts/             ⭐ SSOT — 확정 사실(병입으로만 바뀜)
│     ├─ MVP1차.md · 파이프라인.md · 데이터계약.md · 용어.md
│     └─ 일정.md · 화면.md · 결정로그.md
│
...
| `documents/추천제안.md` | 🤖 AI 추천 제안 (매 증류 재생성 · 루브릭은 distill_prompt) | 증류 엔진 |
