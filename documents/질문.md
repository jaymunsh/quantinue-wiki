...
| A5 🆕 | **실행 추적 테이블 `tb_run` 신설** (지현·오케스트레이터) | cycle_id(판단 묶음)와 별개로 **"코드가 한 번 돈 기록"** 이 없음 — 04~07이 장중 상시로 도는 구조라 수집 실패·지연을 추적할 곳이 필요. 제안: `tb_run(id, job(01~11), started_at, finished_at, status(ok/fail), error, cycle_id nullable)`. solutions의 run_id 개념이 cycle_id로 흡수되며 증발한 부분 (제기: 성혁) |
