...
| A5 🆕 | **실행 추적 테이블 `tb_run` 신설 + cycle_id 배치 확정** (지현·오케스트레이터) | cycle_id(판단 묶음)와 별개로 **"코드가 한 번 돈 기록"** 이 없음 — 04~07이 장중 상시로 도는 구조라 수집 실패·지연을 추적할 곳이 필요. 제안: `tb_run(id, job(01~11), started_at, finished_at, status(ok/fail), error, cycle_id nullable)`. cycle_id를 어느 테이블에 넣는지는 아래 배치표 (제기: 성혁) |
...
| 구간 | 테이블 | cycle_id | 이유 |
|---|---|---|---|
| 발원지 | **tb_strategist_signals** | ✅ **컬럼으로 저장** | 오케스트레이터가 07 실행마다 발급 — 판단 묶음의 시작점 |
| 실행로그 | **tb_run** (신설 시) | ✅ nullable | 07 실행 run만 채움. 01 배치·05 수집 run은 특정 사이클에 안 묶이므로 NULL |
