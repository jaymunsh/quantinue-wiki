...
!!! success "🟢 1차 확정 · 2026-07-06 병입 (결정 #2~#7)"
    기준 = 김지현 「파이프라인 흐름순 상세 명세」(파이프라인·인프라 owner) · 하루 한 사이클이 **01 → 11** 순서로 흐른다.

    - **투자유형** — 공격형 단일 (1차 MVP)
    - **실행** — 전부 가상매매 (Alpaca 페이퍼) · 시드 5인 각 1억

!!! warning "아직 열린 것 (회의 대기)"
    리뷰어 출력 테이블(B8) · 표 접미사(B9) · cycle_id 키(B10) · 모델명 · SQLite 통합시점(A2)
