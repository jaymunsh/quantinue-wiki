...
!!! info "문서 범위"
    이 페이지는 1차 MVP의 01→11 파이프라인을 보존한다. 최종 프로젝트는 이 체인을
    JOB 기반 구조와 장중 WatchRunner로 확장했으며, 현재 요약은 [최종 프로젝트 한눈에](최종프로젝트.md),
    세부 계약은 [최종 통합 설계서](../final/quantinue-integrated-design.html)와
    [JOB·파이프라인](../파이프라인.md)을 따른다.
...
flowchart TB
  ...
  CR -->|"통과한 것만"| RP
  OD --> RV["⑪ 리뷰·회고<br/>복기 → 교훈<br/><i>성혁·구현됨</i>"]
...
- ⚠️ **점수 스케일 전환 협의 중** — conviction 0~10(현 확정) → **0~1 통일**(문턱 5.0 → 0.6)로 바꾸자는 최신안(v3.0)이 확정 대기. 정해지면 이 페이지 숫자를 교체 → [회의 안건](../질문.md)
...
> ⚠️ 뉴스 `confirmed` 게이트는 해당 필드가 삭제되는 방향이라 **재설계 협의 중**, conviction 문턱도 스케일 전환(§2)과 함께 재확정 예정 → [회의 안건](../질문.md)
...
## 6. ⑪ 리뷰·회고 (성혁 담당 · ✅ 구현 완료 · #9·#11)

판단(signal) 1건당 사후 복기 1건. **2단 배치**로 돈다:

1. **매일 적재** — 회고 대기 signal의 T+1~5 종가를 `tb_review_price_snapshots`에 하루 1개씩 (tb_technical은 daily-50이라 탈락 종목 추적 불가 → 별도 스냅샷).
2. **T+5 회고** — 5개 다 모이면 코드(Scorer)가 결과 채점(ret_1/3/5d·hit·mdd) → LLM(Reflector)이 상세 교훈(lesson, 3~5문장) → `tb_review`.

교훈은 다음 사이클 Strategist(⑦)가 참고(같은 실수 반복 억제). **Critic 미참조**(최종 책임은 Strategist signal). **1차 observe-only.** 계약 상세는 `데이터 계약`.
