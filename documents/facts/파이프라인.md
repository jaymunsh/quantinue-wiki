# 🔄 파이프라인 (SSOT)

> **상태: 🟢 1차 확정** — 김지현 「파이프라인 흐름순 상세 명세」(파이프라인·인프라 owner)를 기준으로 확정. 하루 한 사이클이 **01 → 11** 순서로 흐른다.
> 병입 2026-07-06 (결정 #2~#7) · **투자유형 = 공격형 단일** · 전부 가상매매(Alpaca 페이퍼) · 시드 5인 각 1억
> ⚠️ 아직 열린 것(회의): 리뷰어 출력 테이블(B8) · 표 접미사(B9) · cycle_id 키(B10) · 모델명 · SQLite 통합시점(A2)

---

## 1. 한 사이클 — 11단계 전체 흐름

```mermaid
flowchart TB
  subgraph 선별["🔍 종목 선별 (01~03) · 지현·규칙"]
    direction LR
    S1["① 1차 스크리너<br/>top 2,000 유니버스<br/><i>주1회</i>"] --> S2["② 기술 분석<br/>지표 계산<br/><i>매일</i>"]
    S2 --> S3["③ 2차 스크리너<br/>5버킷 → 오늘의 50<br/><i>매일</i>"]
  end

  subgraph 얹기["🌤️ 맥락 얹기 (04~06) · 후보 50에만"]
    direction LR
    MA["④ 매크로<br/>regime·risk_score<br/><i>지현·1시간</i>"]
    DI["⑤ 공시<br/>호재/악재<br/><i>창욱·LLM·1시간</i>"]
    NE["⑥ 뉴스<br/>감성·신뢰<br/><i>창욱·LLM·5분</i>"]
  end

  S3 --> 얹기
  얹기 -->|"각 팀 저장 → Strategist가 SELECT/JOIN"| ST

  subgraph 결정["🧠 결정 (07~08)"]
    direction LR
    ST["⑦ Strategist<br/>매수/보류<br/><i>은미·LLM·장중상시</i>"] --> CR["⑧ Critic<br/>반박·검증<br/><i>미연·LLM</i>"]
  end

  subgraph 실행["🛡️ 실행 (09~10) · 코드"]
    direction LR
    RP["⑨ 리스크·포트폴리오<br/>비중·손절·한도<br/><i>지현</i>"] --> OD["⑩ 주문·체결<br/>Alpaca 페이퍼<br/><i>지현</i>"]
  end

  CR -->|"통과한 것만"| RP
  OD --> RV["⑪ 리뷰·회고<br/>복기 → 교훈<br/><i>성혁·미착수</i>"]
  RV -.->|"tb_review → 다음 판단 참고<br/>(피드백·쿨다운)"| ST

  classDef llm fill:#e9f4ec,stroke:#2C6E52,color:#213
  classDef code fill:#f4efe8,stroke:#8a6d3b,color:#213
  class DI,NE,ST,CR,RV llm
  class S1,S2,S3,MA,RP,OD code
```

> 🟩 초록 = LLM(해석·판단) · 🟫 갈색 = 코드/규칙(계산·강제). **양 끝(스크리너·실행)은 코드, 가운데(분석·판단)만 LLM.**

### 단계 요약표

| # | 단계 | 하는 일 | 산출 테이블 | 주기 | 담당 |
|---|---|---|---|---|---|
| 01 | 1차 스크리너 | NASDAQ 시총 top 2,000 유니버스 | tb_universe | 주1회 | 지현 |
| 02 | 기술 분석 | 캔들 → 지표 계산(rs_20·rsi·macd…) | tb_technical | 매일 | 지현 |
| 03 | 2차 스크리너 | 공통필터 + 5버킷 → 오늘의 50 | tb_daily_pick | 매일 | 지현 |
| 04 | 매크로 | 지수·VIX·금리·달러 → 국면 | tb_macro | 1시간 | 지현 |
| 05 | 공시 | 8-K·10-Q 호재/악재·중요도 | tb_disclosure | 1시간 | 창욱 |
| 06 | 뉴스 | 실시간 뉴스 감성·신뢰도 | tb_news | 5분 | 창욱 |
| 07 | Strategist | 5신호+시세 → 코드게이트+GPT → 매수/보류 | tb_strategist_signals | 장중 상시 | 은미 |
| 08 | Critic | 추천에 반박·검증 | tb_critic_verdict | 매일 | 미연 |
| 09 | 리스크·포트폴리오 | 비중·손절·한도 적용 | (주문 계획) | 매일 | 지현 |
| 10 | 주문·체결 | Alpaca 페이퍼 주문·체결 기록 | tb_account·tb_order·tb_fill | 매일 | 지현 |
| 11 | 리뷰·회고 | 사후 복기 → 교훈 메모리 | tb_review | 매일 | 성혁 |

---

## 2. 안전 구조 — "코드 게이트 샌드위치" (강제규칙 **2개**)

LLM은 앞문·뒷문(코드) 사이에 갇혀 판단한다. GPT가 "매수!"라 우겨도 게이트를 못 넘으면 소용없다.

```mermaid
flowchart LR
  IN[신호 종합] --> FRONT{"🔒 앞문 (코드)<br/>hard_block"}
  FRONT -->|막힘| STOP1["즉시 보류"]
  FRONT -->|통과| MACRO["매크로 감점<br/>(risk-off면 conviction↓)"]
  MACRO --> LLM["🧠 GPT 판단<br/>맥락·확신도(0~10)"]
  LLM --> BACK{"🔒 뒷문 (코드)<br/>min_conviction 5.0"}
  BACK -->|미달| STOP2["보류 강등"]
  BACK -->|통과| BUY["Alpaca 가상 체결"]

  classDef gate fill:#f9ebe8,stroke:#b03a2e,color:#213
  class FRONT,BACK gate
```

- **강제(🔒) 2개**: `hard_block`(상폐·거래정지·파산 등 즉시 보류) · `min_conviction`(공격형 5.0 미달 → 보류 강등).
- **macro_veto 폐지** (결정 #4): 매크로 risk-off는 이제 *거부권이 아니라* conviction **감점**만. 무조건 차단은 `hard_block`뿐.

---

## 3. 데이터 흐름 — Pull 방식 (우편함)

각 팀은 "저장"만, 읽을 필드 선택은 Strategist가 한다. 상류 신호는 **`trade_date`(또는 `collected_at`) + `ticker`** 로 매칭하고, Strategist 이하는 `cycle_id`로 묶는다.

```mermaid
flowchart LR
  subgraph 저장["각 팀 INSERT (후보 50에)"]
    D[(tb_disclosure)]
    N[(tb_news)]
    T[(tb_technical)]
    M[(tb_macro)]
    RVW[(tb_review)]
  end
  D & N & T & M & RVW -->|"SELECT/JOIN (trade_date+ticker · 매크로는 시장전체)"| S["⑦ Strategist"]
  S -->|INSERT| SS[(tb_strategist_signals)]
```

> ⚠️ **키 매핑(B10)**: 상류 테이블은 `trade_date`/`collected_at` 키인데 Strategist는 `cycle_id`로 조회 → 변환 규칙 회의 확정 필요. 필드·타입 상세는 **`데이터 계약`** 페이지.

---

## 4. 확정 원칙 요약

| 원칙 | 내용 |
|---|---|
| Macro-first / 후보 압축 | top 2,000 → 오늘의 50 → 공시·뉴스는 50에만 (비용 방어) |
| Pull 방식 | 각 팀 저장, Strategist가 SELECT/JOIN |
| 코드게이트 샌드위치 | LLM은 제안·해석만, 앞문(hard_block)·뒷문(min_conviction) 코드 강제 |
| 투자유형 | **공격형 단일** (1차 MVP) |
| 실행 | **Alpaca 페이퍼** — 지정가·손절·익절 브래킷 주문, 체결감시는 Alpaca 서버 위임 |
| NO_TRADE 정상 | 안 사는 판단도 근거와 함께 저장 |
| 전부 가상 | 실거래 없음 · 시드 5인 각 1억 |
| 저장소 | Postgres 1개(FK로 촘촘 · 시계열은 TimescaleDB hypertable 후보) |

---

## 5. 세부 정책 (1차 · 백테스트로 튜닝)

**③ 2차 스크리너 — 공통필터 5개 → 5버킷 × 각 10 + 백필 = 50**

| 공통필터 | 기준 | | 버킷 | 정렬키 |
|---|---|---|---|---|
| 주가 최소선 | 종가 ≥ $5 | | 추세 리더 | rs_20 |
| 거래대금 | 20일 평균 ≥ $5M | | 거래량 급증 | ret_5d>0 → vol_ratio |
| ATR 상한 | ≤ 종가 15% | | 신고가 돌파 | high_252_ratio≥0.95 |
| 방향 게이트 | 종가 > ma20 | | 눌림목 | pullback → rs_20 |
| 지속성 | 20일 수익률 > 0 | | 스퀴즈 돌파 | 밴드폭≤0.25 → vol_ratio |

**④ 매크로 — risk_score(0~100)** = `0.40·VIX + 0.30·지수등락 + 0.15·금리 + 0.15·달러` · regime: `≤30 risk_on / 30~70 neutral / ≥70 risk_off`

**⑦ 공격형 POLICY 문턱**: 공시 sent≥0.60·imp≥0.25 · 뉴스 sent≥0.60·grade≥0.50·confirmed≥0.40 · 필요 합의 **2/4** · min_conviction **5.0**

**⑨ 공격형 포트폴리오(aggressive.yaml)**: 한 종목 최대 **25%** · 목표 **5종목** · 손절 **−15%** · 1회 리스크 **4%** · 주식 비중 100% · risk_off면 신규 매수 축소

**⏰ 실행 타이밍**: 배치(01~03)는 KST 애프터마켓 종료~데이마켓 시작 **10분 공백(08:50~09:00)** · 04~07은 장중 상시(매크로 1h·공시 1h·뉴스 5분·전략 상시)

---

## 6. ⑪ 리뷰·회고 — 내(성혁) 파트 (미착수)

> **기준 = 이 파이프라인 명세의 `tb_review`** 에서 새로 설계 시작. (이전 성혁 초안은 무시 — `데이터 계약` §4 참조)

사이클 결과를 사후 복기 → 교훈(lesson)·통계(stats)를 `tb_review`에 남기고, 다음 사이클 Strategist(⑦)로 피드백(같은 실수 반복 억제 + 최근 기각 쿨다운). **1차는 observe-only.**

---

## 7. 아직 열린 안건 (회의)
B8 리뷰어 출력 테이블(tb_review ↔ 은미 tb_memory_entries) · B9 `_signals` 접미사 · B10 cycle_id↔trade_date 키 · LLM 모델명 · A2 SQLite→Postgres 통합 시점 → `데이터 계약` · `회의 안건` 참조
