...
> **상태: 🟡 부분 확정 (2026-07-06).** 파이프라인 명세 기준 **A1 투자유형(공격형 단일)·A4 risk_score(0~100)는 확정**(결정 #2·#6). 나머지 필드 계약(B8~B10 등)은 회의 확정 전. schema keeper(성혁)가 은미·창욱·지현 스키마를 하나로 맞춘 제안.
...
> ⚠️ **B8 — 은미 소비 스키마와 불일치**: 은미의 07단계는 `tb_memory_entries`(side·conviction·outcome·lesson·created_at)를 읽는다. **`tb_review`(내 회고 원본)와 `tb_memory_entries`(은미 입력)가 같은 표인지 / 2단(원본→뷰)인지** 회의 확정 필요. 내 제안: `tb_review`=원본, `tb_memory_entries`=은미용 계약 뷰로 둘 다 유지. **1차는 observe-only.**
