...
!!! warning "1차 계약 보존본"
    이 페이지의 14개 테이블·필드 설명은 1차 설계 당시 계약이다. 최종 프로젝트 원장은
    PostgreSQL 28테이블이며 원문 원장·집계 신호·JOB 실행·장중 사건 계층이 추가됐다.
    현재 구조의 요약은 [최종 프로젝트 한눈에](최종프로젝트.md), 기계적 정본은
    이 위키에 보존한 [schema.sql](../final/reference/schema.sql)과
    [pipeline.yaml](../final/reference/pipeline.yaml)이다.

!!! abstract "🟡 부분 확정 (2026-07-06) · 14개 테이블"
...
erDiagram
  ...
  tb_strategist_signals ||--o{ tb_review_price_snapshots : "signal별 T+1~5 종가"
  tb_review_price_snapshots ||--o| tb_review : "5개 다 모이면 회고 1건"
```
...
| 11a | [tb_review_price_snapshots](#tb_review_price_snapshots) | 회고용 T+1~5 종가 (signal별) | 문성혁 | 매일 적재 | 🟢 확정 (#11) |
| 11b | [tb_review](#tb_review) | 회고 (결과 채점·교훈) | 문성혁 | T+5 생성 | 🟢 확정 (#9·#11 · 구현 착수) |
...
| 항목 | 내용 | 상태 |
|---|---|---|
| 저장소 | **Postgres 1개** — 표들이 FK로 촘촘히 엮여(대부분 tb_daily_pick 조인) 두 DB로 나누면 FK가 깨짐. 시계열(🕐)은 TimescaleDB hypertable 후보. candle은 메모리/CSV 캐시(DB 아님) | 🟢 ✅ 결정 #12 |
...
| 점수 스케일 | **신호 점수 0~1** · **conviction·문턱 0~10** (서로 다름 주의!) · 매크로 risk_score만 **0~100** (✅ 결정 #6) | ⚠️ conviction도 0~1로 통일하자는 최신안(v3.0) 확정 대기 → [회의 안건](../질문.md) |
...
**회고용 T+1~5 종가 (중간 테이블)** · 갱신 2026-07-06 (결정 #11)
👤 소유자 **문성혁** · 🔁 매일 장마감 배치 — 회고 대기 signal의 종가를 하루 1개씩 적재 · 🔑 PK `(signal_id, day_offset)`
**왜 별도 테이블인가:** `tb_technical.close`는 그날 **오늘의 50만** 담는다 → 판단한 종목이 T+1/3/5에 50 밖으로 빠지면 종가를 못 구함. 그래서 Strategist가 실제 판단한 signal만 T+1~5 종가를 따로 추적한다(전체 유니버스 저장 아님).

| 컬럼 | 타입 | 키 | 설명 |
|---|---|---|---|
| signal_id | BIGINT | 🔑🔗 | → tb_strategist_signals.id |
| day_offset | INT | 🔑 | 1~5 (판단일 기준 영업일 수) |
| price_date | DATE | | 그 종가의 실제 날짜 |
| close | NUMERIC | | 종가 |
| source | TEXT | | 출처 (yfinance/Finnhub 등) |
| created_at | TIMESTAMPTZ | | 적재 시각 (idempotent upsert — 같은 날 재실행해도 같은 행만 갱신) |

> **적재 배치(매일):** 회고 없는 signal 중 오늘이 T+1~5인 것만 종가 조회 → upsert. LLM·채점 호출 안 함. 5개 다 모이면 그 signal은 회고 가능 상태.

??? example "샘플 — signal 987이 T+1~5 쌓이는 모습"
    | signal_id | day_offset | price_date | close | source |
    |---|---|---|---|---|
    | 987 | 1 | 2026-07-07 | 101.20 | yfinance |
    | 987 | 2 | 2026-07-08 | 99.50 | yfinance |
    | 987 | 3 | 2026-07-09 | 98.00 | yfinance |
    | 987 | 4 | 2026-07-10 | 102.00 | yfinance |
    | 987 | 5 | 2026-07-13 | 103.10 | yfinance |

    `close`는 각 날짜의 **실제 종가**(P0 대비 변환값 아님). P0(매수=체결가 $100)는 Scorer가 수익률 계산 때 적용 → ret_5d = (103.10−100)/100 = **+3.1%**.

### tb_review

**회고 (결과 채점 + 교훈)** · 갱신 2026-07-06 (담당 확정 · 결정 #9·#11 · 구현 착수)
👤 소유자 **문성혁** · 🔁 T+5 배치 — snapshot 5개 모인 판단을 확정 채점 · 🔑 PK `(id)` · 🔗 FK signal_id→tb_strategist_signals
**판단 데이터는 복사하지 않는다** — signal_id JOIN으로 side·conviction·근거 전부 도달. T+1~5 종가는 `tb_review_price_snapshots`에서 JOIN. ⑪이 새로 만드는 값(결과·교훈)만 저장. **1차 observe-only.**
**Critic 미참조** — 최종 판단 책임은 Strategist signal에 있으므로(초안→Critic 반박→Strategist 최종) 회고는 최종 결정만 평가. Critic 품질 평가는 2차 별도 축.
...
| lesson | TEXT | | **상세 교훈 3~5문장** — Reflector(LLM) 생성. 판단근거 + T+1/3/5 흐름 + 다음 행동 규칙 포함, 숫자 인용 필수·일반론 금지. (한 줄 아님 — 다음 Strategist가 메모리로 재사용하려면 맥락이 필요) |
| created_at | TIMESTAMPTZ | | 기록 시각 — **완전 insert-once**(Scorer+Reflector 한 배치 완성, 실패 시 통째 재시도)라 updated_at 없음 |

??? example "샘플 데이터 + lesson 예시 (상세)"
    | id | signal_id | ret_1d | ret_3d | ret_5d | hit | max_drawdown |
    |---|---|---|---|---|---|---|
    | 41 | 987 | +1.2 | −2.0 | +3.1 | true | −2.0 |

    **lesson (id 41):** "NVDA buy 판단은 T+1 +1.2%, T+3 −2.0%, T+5 +3.1%로 최종 적중. 중간 −2.0% 눌림에도 key_risk였던 단기 과열은 실제 훼손으로 이어지지 않았고, 실적·수급 bull_case가 T+5까지 설명력을 유지했다. 다음 판단에서 실적·수급 근거가 유지되는 buy는 T+3 수준 단기 조정만으로 철회하지 않는다."

!!! note "남은 확인 (회의)"
    은미 소비 스펙 충족 확인(B8) · NO_TRADE 회고 범위·1차 메모리 주입 여부(B11-③④) → **[회의 안건](../질문.md)**. (P0·T+5·Critic 미참조는 #11로 확정)
