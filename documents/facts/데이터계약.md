...
| 11 | [tb_review](#tb_review) | 회고 (교훈·통계) | 문성혁 | 매일 | 🔘 설계 예정 |
...
| **공통 필수 컬럼** | 모든 테이블에 `created_at`(행 기록 시각) + `updated_at`(행 수정 시각) **필수** — 감사·디버깅·동기화 추적용. updated_at은 안 쓰이는 표가 있어도 통일해서 둔다 (✅ 결정 #8 · schema keeper 표준) | 🟢 |
...
> 공통 필수 컬럼 `created_at`·`updated_at`(결정 #8)은 모든 테이블에 있음 — 아래 표에는 created_at만 표기하고 updated_at은 생략.
...
**회고 (교훈·통계)** 🔘 설계 중
👤 소유자 **문성혁** · 🔁 매일 · 🔑 PK `(id)` · 🔗 FK→tb_daily_pick
...
!!! warning "담당(성혁) 개정안 제출 — 회의 확정 대기"
    위 표는 파이프라인 명세의 초안(지현 스스로 "7~11 컬럼은 담당이 확정"으로 위임). 담당 개정안(2026-07-06): **signal_id FK 중심 + 결과 컬럼(ret_1/3/5d·hit·mdd) + lesson**, `tb_memory_entries`는 폐기하고 JOIN으로 은미 스펙 충족. 상세·채점 공식은 → **[회의 안건](../질문.md)** B8·B11. 확정(병입) 시 위 표가 개정안으로 교체된다.
