...
erDiagram
  ...
  tb_daily_pick ||--o{ tb_review : "회고 → 다음 판단 피드백"
```
...
| # | 테이블 | 무엇 | 소유 | 주기 | 상태 |
|---|---|---|---|---|---|
| 01 | [tb_universe](#tb_universe) | 감시 유니버스 2,000 | 지현 | 주1회 | 🟢 확정 |
| 02 | [tb_technical](#tb_technical) | 기술 지표 (오늘의 50) | 지현 | 매일 | 🟢 확정 |
| 03 | [tb_daily_pick](#tb_daily_pick) | 오늘의 50 선정 결과 | 지현 | 매일 | 🟢 확정 |
| 04 | [tb_macro](#tb_macro) | 시장 국면 (시장 전체) | 지현 | 1시간 | 🟢 확정 |
| 05 | [tb_disclosure](#tb_disclosure) | 공시 분석 | 창욱 | 1시간 | 🟡 필드 협의 |
| 06 | [tb_news](#tb_news) | 뉴스 분석 | 창욱 | 5분 | 🟡 필드 협의 |
| 07 | [tb_strategist_signals](#tb_strategist_signals) | 전략 판단 (매수/보류) | 은미 | 장중 상시 | 🟡 조율 중 |
| 08 | [tb_critic_verdict](#tb_critic_verdict) | 반박·검증 | 미연 | 매일 | 🔘 설계 예정 |
| 09~10 | [tb_account · tb_order · tb_fill](#tb_account-tb_order-tb_fill) | 계좌·주문·체결 | 지현 | 매일 | 🔘 설계 예정 |
| 11 | [tb_review](#tb_review) | 회고 (교훈·통계) | 성혁 | 매일 | 🔘 설계 예정 |
...
| created_at | 모든 테이블 공통 — 행이 DB에 기록된 시각 (감사·디버깅용) | 🟢 |
...
**감시 유니버스 2,000** 🕐 · 소유 지현 · **주1회** 갱신 · PK `(as_of_date, ticker)`
...
**오늘의 50 (선정 결과)** 🕐 · 소유 지현 · **매일** · PK `(trade_date, ticker)` · FK→tb_universe
...
**기술 지표 (오늘의 50)** 🕐 · 소유 지현 · **매일** · PK `(trade_date, ticker)` · tb_daily_pick과 **1:1**
...
**시장 국면 (시장 전체 — ticker 없음)** 🕐 · 소유 지현 · **1시간** · PK `(as_of)`
...
**공시 분석** 🕐 · 소유 창욱 · **1시간 간격, 새 공시만 append** · PK `(ticker, collected_at)` · FK→tb_daily_pick
...
**뉴스 분석** 🕐 · 소유 창욱 · **5분 간격, 새 기사만 append** · PK `(ticker, collected_at)` · FK→tb_daily_pick
...
**전략 판단 (매수/보류)** · 소유 은미 · **장중 상시** · PK `(id)` · FK cycle_id · ticker→tb_daily_pick
...
**반박·검증** · 소유 미연 · 🔘 설계 예정 · PK `(id)` · FK→tb_strategist_signals
...
**계좌·주문·체결 (Alpaca 페이퍼)** · 소유 지현 · 🔘 설계 예정 — Alpaca API 연동 후 컬럼 확정

| 테이블 | 담는 것 | 키 |
|---|---|---|
| tb_account | 5인 각 계좌 잔고·보유 (시드 각 1억 상당) | 🔑 id |
| tb_order | 매수/매도 주문 (종목·수량·유형) | 🔑 id · 🔗 tb_strategist_signals·tb_account |
| tb_fill | 체결 기록 (체결가·수량·시각) | 🔑 id · 🔗 tb_order |

### tb_review

**회고 (교훈·통계)** · 소유 **성혁** · 🔘 설계 예정 · PK `(id)` · FK→tb_daily_pick
사이클 결과를 사후 복기 → 교훈·통계를 남기고 다음 사이클 Strategist로 피드백. **1차는 observe-only**(기록만).
...
!!! danger "은미 소비 스키마와 불일치 (회의 확정 필요)"
    Strategist(07)는 `tb_memory_entries`(side·conviction·outcome·lesson)를 읽는다고 돼 있어 **이름·컬럼이 `tb_review`와 다름.** 통일 방안(같은 표 vs 원본→뷰 2단)은 → **[회의 안건](../질문.md)** 의 "리뷰어 테이블" 항목 참조.
...
## 4. 공유 온톨로지 — event_type 11종 (공시·뉴스 공통, 소유 창욱)
