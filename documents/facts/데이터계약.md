...
!!! abstract "🟡 부분 확정 (2026-07-06)"
    schema keeper(성혁)가 은미·창욱·지현 스키마를 하나로 맞춘 계약. 확정된 값은 그대로 쓰고, **⚠️ 표시 필드만 회의에서 결정**하면 완전 확정된다 → 미확정 목록은 **[회의 안건](../질문.md)**.
...
## 4. 🔗 리뷰어 계약 — ⑪ 회고 (성혁 담당 · 설계 예정)

리뷰어 파트는 설계 시작 전. **파이프라인 명세의 `tb_review`** 를 출발점으로 설계한다.

**기준 = `tb_review`** (지현 ERD · ⑪ 산출)
...
확정된 결정과 이유는 **[결정 로그](결정로그.md)**, 남은 결정(⚠️ 표시 필드들)의 상세와 논의 배경은 **[회의 안건](../질문.md)** 한 곳에서만 관리한다. 회의에서 남은 항목만 정하면 이 문서가 **완전 확정 데이터 계약**이 된다.
