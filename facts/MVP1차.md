...
> **상태: 🟡 초안(제안).** 파이프라인 구조는 그대로 두고, "1차에 **실제로 만들 것**"만 잘라낸 영역. 이 문서가 5명의 **구현 기준선**이다.
> **왜 필요한가:** 설계서는 1차·2차가 섞여 있어 "지금 뭘 만들지"가 흐릿하다(멘토도 "MVP 완료 조건 불명확" 지적). 여기서 **IN/OUT을 못 박아** 그 토대로 개발한다.

---

## 1. MVP 1차 = 한 문장

> **"공격형 1종으로, 오늘의 후보를 골라 → 분석·판단·반박·리스크검문 → 가상 체결하고 → 회고를 기록하는 한 사이클이, 로컬에서 처음부터 끝까지 도는 것."**
> 수익이 목표가 아니라 **자율 루프 완주 + 근거가 다 남는 것**이 성공. (전부 가상 PaperBroker · 실제 돈 0)
...
> ⭐ **핵심 컷 2개:** ①**ML은 1차 매매에 안 씀**(설계서 원칙 — 은미 로직에서 ml_prob_up 조건 제거, trend만으로 기술 카운트) ②**투자유형 1종만**.
