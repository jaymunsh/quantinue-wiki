...
Reviewer는 Strategist가 남긴 최종 판단을 T+5 이후에 복기하는 모듈이다. 숫자는 코드가 계산하고, LLM은 이미 계산된 숫자를 바탕으로 lesson 한 줄만 만든다.
...
flowchart TD
    ...
    AGENT["agent.py<br/>LLM lesson 생성"]
...
| `agent.py` | Scorer 결과를 LLM lesson으로 바꾸고 `TbReviewDraft` 생성 |
...
4. `reflect_review()`가 LLM lesson을 만든다.
...
이 모델을 둔 이유는 Reviewer가 전체 가격 테이블을 소유하지 않기 때문이다. 상위 스케줄러가 Strategist가 실제 판단한 signal에 대해서만 T+1~T+5 종가를 모아 넘겨준다.
...
`tb_review`에 저장할 값을 담은 Python 객체다. 실제 `tb_review_draft` 테이블은 없다.
...
중요한 점은 LLM이 숫자를 계산하지 않는다는 것이다. LLM은 `ReviewScore`에 이미 들어 있는 숫자를 보고 lesson만 쓴다.
...
flowchart TD
    ...
    REFLECT["reflect_review()<br/>lesson 생성"]
